...
function handleError(event, err) {
    try {
        const msg = document.getElementById('noxdd_fallback_errormsg');
        ...
        div.classList.add('noxdd_message');
        div.appendChild(document.createTextNode(event.message));
        msg.appendChild(div);

        if (event.filename) {
            div = document.createElement("div");
            div.className = 'noxdd_location';
        ...
        if (err && err.stack) {
            div = document.createElement("div");
            div.className = 'noxdd_stack';
            div.appendChild(document.createTextNode(err.stack));
            msg.appendChild(div);
        }

        document.getElementById('noxdd_fallback_error')
            .classList.add("noxdd_open");

    } catch (exc) {
        document.write("noxdd encountered an error.");
    }
