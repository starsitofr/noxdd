 ...
 * Copyright (C) 2020 The noxdd Authors
...
export default class RFB extends EventTargetMixin {
    constructor(target, urlOrChannel, options) {
        ...
        if (!window.isSecureContext) {
            Log.Error("noxdd requires a secure context (TLS). Expect crashes!");
        }
        ...
        // XXX: Tightxdd 2.8.11 sends no cursor at all until Windows changes
        // it. Result: no cursor at all until a window border or an edit field
        // is hit blindly. But there are also xdd servers that draw the cursor
        // in the framebuffer and don't send the empty local cursor. There is
        // no way to satisfy both sides.
        //
        // The spec is unclear on this "initial cursor" issue. Many other
        // viewers (Tigerxdd, Realxdd, Remmina) display an arrow as the
    ...
    _socketOpen() {
        if ((this._rfbConnectionState === 'connecting') &&
            ...
            Log.Debug("Starting xdd handshake");
        } else {
    ...
    _handleWheel(ev) {
        ...
        // Mouse wheel events are sent in steps over xdd. This means that the xdd
    ...
    _negotiateProtocolVersion() {
        ...
        let isRepeater = 0;
        switch (sversion) {
            case "000.000":  // Ultraxdd repeater
                isRepeater = 1;
                break;
            case "003.003":
            case "003.006":  // Ultraxdd
            ...
            case "004.001":  // Realxdd 4.6
            case "005.000":  // Realxdd 5.3
    ...
    _negotiateXvpAuth() {
        ...
        this._rfbAuthScheme = securityTypexddAuth;
    ...
    _negotiateTightTunnels(numTunnels) {
        ...
        // Siemens touch panels have a xdd server that supports NOTUNNEL,
    ...
    _negotiateTightAuth() {
        if (!this._rfbTightxdd) {  // first pass, do the tunnel negotiation
            if (this._sock.rQwait("num tunnels", 4)) { return false; }
            const numTunnels = this._sock.rQshift32();
            if (numTunnels > 0 && this._sock.rQwait("tunnel capabilities", 16 * numTunnels, 4)) { return false; }

            this._rfbTightxdd = true;
    ...
    _negotiateAuthentication() {
        switch (this._rfbAuthScheme) {
            ...
            case securityTypexddAuth:
                return this._negotiateStdxddAuth();
    ...
    _handleSecurityResult() {
        if (this._sock.rQwait('xdd auth response ', 4)) { return false; }
    ...
    _negotiateServerInit() {
        ...
        if (this._rfbTightxdd) {
            if (this._sock.rQwait('Tightxdd extended server init header', 8, 24 + nameLength)) { return false; }
            // In Tightxdd mode, ServerInit message is extended
            ...
            if (this._sock.rQwait('Tightxdd extended server init header', totalMessagesLength, 32 + nameLength)) { return false; }
