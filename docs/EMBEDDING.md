# Embedding and Deploying noxdd Application

This document describes how to embed and deploy the noxdd application, which
includes settings and a full user interface. If you are looking for
documentation on how to use the core noxdd library in your own application,
then please see our [library documentation](LIBRARY.md).

## Files

The noxdd application consists of the following files and directories:

* `xdd.html` - The main page for the application and where users should go. It
  is possible to rename this file.

* `app/` - Support files for the application. Contains code, images, styles and
  translations.

* `core/` - The core noxdd library.
...
server and setting up a WebSocket proxy to the xdd server.

## Parameters

The noxdd application can be controlled by including certain settings in the
...
* `reconnect` - If noxdd should automatically reconnect if the connection is
  dropped.
...
* `repeaterID` - The repeater ID to use if a xdd repeater is detected.

* `shared` - If other xdd clients should be disconnected when noxdd connects.
...
If you serve noxdd files using a web server that provides an ETag header, and
...
between the new xdd.html (which is reloaded because the user has used it with
...
```
    ...
    # In the <Directory> or <Location> block related to noxdd
...
```
    # In the location block related to noxdd
