/* global xdd_frame_data, xdd_frame_encoding */
...
function enableUI() {
    ...
    message("Loaded " + xdd_frame_data.length + " frames");
    ...
    /* eslint-disable-next-line camelcase */
    frames = xdd_frame_data;

    let encoding;

    /* eslint-disable camelcase */
    if (window.xdd_frame_encoding) {
        // Only present in older recordings
        encoding = xdd_frame_encoding;
...
function start() {
    ...
    player.onrfbdisconnected = (evt) => {
        if (!evt.detail.clean) {
            message(`noxdd sent disconnected during iteration ${evt.detail.iteration} frame ${evt.detail.frame}`);
