 ...
 * Copyright (C) 2018 The noxdd Authors
...
if (window.setImmediate === undefined) {
    ...
    window.setImmediate = (func) => {
        ...
        window.postMessage("noxdd immediate trigger:" + index, "*");
        return index;
    };
    ...
    window.addEventListener("message", (event) => {
        if ((typeof event.data !== "string") ||
            (event.data.indexOf("noxdd immediate trigger:") !== 0)) {
            return;
        }

        const index = event.data.slice("noxdd immediate trigger:".length);
...
export default class RecordingPlayer {
    ...
    run(realtime, trafficManagement) {
        ...
        this._rfb = new RFB(document.getElementById('xdd_screen'), this._ws);
